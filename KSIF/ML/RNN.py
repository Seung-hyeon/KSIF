...
참조 : http://aikorea.org/blog/rnn-tutorial-1/
"""
import numpy as np
import theano
...
def test_real():
    ...
    model = MetaRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
                    ...
                    n_epochs=4, activation='tanh')

    model.train(seq, targets, validation_frequency=1000)
...
def test_price():
    """ Test RNN with binary outputs. """
    import KSIF as kf
    price = kf.generate.prices('A00')

    n_hidden = 10
    n_in = 5
    n_out = 1
    n_steps = len(price)
    n_seq = len(price.columns)
    n_epochs = 5
    # simple lag test
    """
    seq = np.random.randn(n_seq, n_steps, n_in)
    targets = np.zeros((n_seq, n_steps, n_out))"""
    seq = []
    for s in price.values.T:
        buf = []
        for i in range(len(s)-n_in+1):
            buf.append(s[i:i+n_in])
        seq.append(buf)

    seq = np.array(seq)
    targets = np.zeros((n_seq, n_steps, n_out))
    # whether future is greater than present
    targets[:, :-1, 0] = np.cast[np.int](seq[:, 1:, 0] > seq[:, :-1, 0])
