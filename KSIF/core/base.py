...
class AlgoStack(Algo):
    ...
    def __init__(self, *algos):
        ...
        self.algos = list(algos)  # to modify
...
class Strategy(StrategyBase):
    ...
    Attributes:
        ...
        * temp (dict): A dict cont()aining temporary data - cleared on each call
    ...
    def build_port(self, prices):
        ...
        for dt in prices.index:
            ...
            if not self.bankrupt:
                self.run()
                self.update(dt)
            if last_temp != self.temp:
                if first:
                    buf = pd.Series({'cash': 1}).to_frame('weights')
                    buf['DATE'] = dt - (prices.index[1] - prices.index[0])
                    buf['ret'] = 0
                    port = port.append(buf)
                    first = False
                else:
                    now_prices = prices.ix[dt]
                    buf['ret'] = now_prices / last_prices - 1
                    port = port.append(buf)

                if self.temp != {}:
                    buf = pd.Series(self.temp['weights']).to_frame('weights')
                    buf['DATE'] = dt

                last_temp = self.temp
                last_prices = prices.ix[dt]

        port = port.reset_index().rename(columns={'index': 'firmco'}).set_index('DATE')

        # drop Nan weight values
        port = port[np.isfinite(port['weights'])]

        # if ret is Nan -> delisting -> ret = -100%
        port['ret'] = port['ret'].fillna(-1)
