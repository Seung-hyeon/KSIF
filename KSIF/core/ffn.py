...
class PerformanceStats(object):
    ...
    def _calculate(self, obj):
        ...
        self.daily_prices = obj.resample('d').interpolate().resample('B').interpolate()
        # M = month end frequency
        self.monthly_prices = obj.resample('M').interpolate()
        # A == year end frequency
        self.yearly_prices = obj.resample('A').interpolate()
        ...
        dr = self.daily_prices.to_returns()

        if len(self.returns) < 2:
            return

        self.daily_mean = dr.mean() * 252
        self.daily_vol = dr.std() * np.sqrt(252)
        self.daily_sharpe = (self.daily_mean - self._daily_rf) / self.daily_vol
        self.best_day = dr.max()
        self.worst_day = dr.min()
        ...
        self.daily_skew = dr.skew()

        # if all zero/nan kurt fails division by zero
        if len(dr[(~np.isnan(dr)) & (dr != 0)]) > 0:
            self.daily_kurt = dr.kurt()
    ...
    def plot_histogram(self, period='m', figsize=(15, 5), title=None,
        ...
        plt.figure(figsize=figsize)
        ax = ser.hist(bins=bins, figsize=figsize, normed=True, color='gray', **kwargs)
        ax.set_title(title)
...
class GroupStats(dict):
    ...
    def display(self):
        ...
        logging.warning("  * Returns are calculated based on resampled interpolated data " +
                        "which may cause extremely low volatility\n")
        data = []
        first_row = ['Stat']
    ...
    def plot(self, period='d', figsize=(15, 5), title=None,
        ...
        if period == 'y':
            period = 'a'
        ser = self.prices.asfreq(period).rebase()
        # Add interpolation, because there can be different frequency data.
        ax = ser.interpolate().plot(figsize=figsize, logy=logy, title=title, **kwargs)
...
def rebase(prices, value=100):
    ...
    # 시작값이 다를 수 있기 때문에 다른 rebase를 해야함
    df = pd.DataFrame({})
    for column in prices:
        first_value = prices[column].loc[prices[column].first_valid_index()]
        df[column] = prices[column] / first_value * value
    return df


def normalize(prices, value=100):

    return rebase(prices, value)
...
def calc_port_return(port, start='1700-01-01', end='2200-01-01', dates=None, eq_weight=True):
    """
    Calculates the portfolio return.

    :param port: (pd.DataFrame) portfolio which have each return and weight
    :return: (pd.DataFrame) portfolio return
    """
    port.index = port.index.rename('DATE')
    port = port.reset_index()

    if dates is None:
        port = port[(port.DATE > start) & (port.DATE <= end)]
    else:
    ...
    if eq_weight:
        g = g['ret'].mean().to_frame()

    else:
        # Weighted Average
        port['wa'] = port.ret / g.weights.transform('sum') * port.weights
        g = g.wa.sum().to_frame('ret')

    g['cumret'] = (1 + g['ret']).cumprod()
