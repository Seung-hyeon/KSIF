...
from KSIF.core.utils import parse_arg
import pandas as pd
import numpy as np
...
class Backtest(object):
    ...
    def _portfolios(self, date=None):
        ...
        """
        if date is None:
            stk = self.positions.stack()
            stk = stk[stk != 0]
            return stk

        elif isinstance(date, int):
            port = self.positions.ix[date]
            port = port[port != 0]
            return port.dropna()
            ...
            return self._portfolios(idate).dropna()

    @property
    def portfolios(self):
        return self._portfolios()

    def portfolio(self, date=None):
...
class Result(ffn.GroupStats):
    ...
    def __init__(self, *backtests):
        ...
        if len(backtests) == 1:
            self.strategy = self.backtest_list[0].strategy
        else:
            self.strategy = {x.name: x.strategy for x in backtests}

    def __add__(self, other):
        new_backtests = [test for test in self.backtest_list]
        new_backtests += [test for test in other.backtest_list]
        return Result(*new_backtests)

    def __sub__(self, other):
        new_backtests = [test for test in self.backtest_list]
        new_backtests = list(set(new_backtests) - set([test for test in other.backtest_list]))
        return Result(*new_backtests)

    def sub(self, other_name):
        others = parse_arg(other_name)
        new_backtests = [test for test in self.backtest_list if test.name not in others]
        return Result(*new_backtests)

    def subtract(self, other_name):
        return self.sub(other_name)
